...
Optimizes directly against the individual crime incidents (no aggregation).

Pipeline
--------
Stage 1  Weighted Mini-Batch K-Means on crime incidents -> candidate set J
...
NUM_SECTORS = 36         # P    command centres to locate
...
def solve_ppac_ip(coverage_sets: list,
    ...
    """
    Solve the Exact PPAC Formulation directly against the individual crime incidents.
    """
    ...
    print(f"    Building {n_inc:,} coverage constraints (this will take a while and consume memory)...")
    ...
    for i in range(n_inc):
        ...
        # 5% Progress Tracker for Constraint Building
...
def generate_patrol_map():
    ...
    # We now ONLY build the beat-to-incident sets, evaluating against the real crime points
    ...
    # Using inc_coverage_sets and weights directly against n_inc
