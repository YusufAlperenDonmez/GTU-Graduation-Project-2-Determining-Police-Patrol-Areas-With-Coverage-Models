...
# How many decimal places of weight precision to preserve.
# 1e4 keeps 4 decimal places (e.g. 2.3456 -> 23456).
# Increase if your weights have more significant fractional digits.
WEIGHT_SCALE = 10_000
...
OUTPUT_IMG  = '../outputs/optimized/backup/ppac_exact_optimal_2.15m_300beats.png'
OUTPUT_CSV  = '../outputs/optimized/backup/ppac_exact_summary_2.15m_300beats.csv'
OSM_CACHE   = '../resources/la_drive_network.graphml'

# ── NEW: UI Export paths ──────────────────────────────────────────────────────
OUTPUT_STATIONS    = '../outputs/optimized/backup/stations_2.15m_300beats.csv'
OUTPUT_INCIDENTS   = '../outputs/optimized/backup/incidents_export_2.15m_300beats.csv'
OUTPUT_OPT_SUMMARY = '../outputs/optimized/backup/optimization_summary_2.15m_300beats.csv'
OUTPUT_BEATS_GEO   = '../outputs/optimized/backup/beat_polygons_2.15m_300beats.geojson'
...
def solve_ppac_ip(coverage_sets: list,
    ...
    """
    Solves the PPAC multi-coverage IP using OR-Tools CP-SAT.
 
    CP-SAT requires all coefficients to be integers.  Float weights are scaled
    by WEIGHT_SCALE and rounded.  The returned objective value and gap are
    rescaled back to the original units so nothing downstream changes.
 
    Parameters / return signature are identical to the original SCIP version.
    """
    ...
    int_min_cov = int(np.floor(min_coverage_score * WEIGHT_SCALE))
 
    print(f"    CP-SAT solver | {n_cand} candidates | {n_inc} incidents "
          f"| P={P} | {n_threads} threads")
    print(f"    Weight scale: {WEIGHT_SCALE}x  "
          f"(min_coverage threshold -> {int_min_cov:,})")
 
    # ── Build reverse mapping N[i] = list of candidate indices that cover i ───
    print("    Building incident->candidate reverse mapping...")
    ...
    x = [model.NewBoolVar(f'x_{j}') for j in range(n_cand)]
 
    # y[i] = number of selected stations covering incident i  (0..P)
    # Using IntVar here (not BoolVar) to capture multi-coverage counts
    y = [model.NewIntVar(0, P, f'y_{i}') for i in range(n_inc)]
 
    # z[i] = 1 if incident i is covered by at least one station
    z = [model.NewBoolVar(f'z_{i}') for i in range(n_inc)]
    ...
    for i in range(n_inc):
        if N[i]:
            # y[i] == sum of x[j] for all j that cover i
            model.Add(y[i] == sum(x[j] for j in N[i]))
 
            # z[i] == 1  <=>  y[i] >= 1
            # Implemented as:
            #   y[i] >= z[i]          (if z=1 then y>=1)
            #   y[i] <= P * z[i]      (if y>=1 then z=1)
            model.Add(y[i] >= z[i])
            model.Add(y[i] <= P * z[i])
        else:
            # Incident unreachable by any candidate — lock to zero
            model.Add(y[i] == 0)
            model.Add(z[i] == 0)
 
        if (i + 1) % print_interval == 0 or (i + 1) == n_inc:
            pct = 100 * (i + 1) / n_inc
            print(f"       Constraint build: {pct:.0f}%  ({i+1}/{n_inc})")
 
    # Exactly P stations selected
    model.Add(sum(x) == P)
 
    # Optional: minimum primary (once-covered) weighted score
    if int_min_cov > 0:
        print(f"    Adding min-coverage floor: {int_min_cov:,} (scaled)")
        model.Add(
            sum(int(int_weights[i]) * z[i] for i in range(n_inc)) >= int_min_cov
        )
 
    # Objective: maximise sum of weight[i] * y[i]  (backup-weighted coverage)
    model.Maximize(
        sum(int(int_weights[i]) * y[i] for i in range(n_inc))
    )
 
    # ── Solver parameters ─────────────────────────────────────────────────────
    solver = cp_model.CpSolver()
    solver.parameters.num_workers = n_threads          # use all cores
    solver.parameters.log_search_progress = True       # mirrors EnableOutput()
    ...
    # CP-SAT-specific tuning for large binary MIPs:
    # LNS (large neighbourhood search) port is already on by default.
    # linearization_level=2 enables stronger LP relaxation cuts.
    solver.parameters.linearization_level = 2
 
    print(f"    Solving with CP-SAT ({n_threads} workers)...")
    ...
    # ── Extract solution and rescale objective back to original units ─────────
    obj_val_scaled = solver.ObjectiveValue()
    obj_val = obj_val_scaled / WEIGHT_SCALE          # back to float-weight units
    print(f"    Objective Z*  : {obj_val:,.2f}  (scaled: {obj_val_scaled:,.0f})")
 
    x_sol = np.array([solver.Value(x[j]) for j in range(n_cand)], dtype=bool)
    y_sol = np.array([solver.Value(y[i]) for i in range(n_inc)], dtype=float)
 
    # MIP gap: (best_bound - best_obj) / best_obj
    best_bound = solver.BestObjectiveBound() / WEIGHT_SCALE
    gap = abs(best_bound - obj_val) / max(1e-9, abs(obj_val))
 
    return x_sol, y_sol, obj_val, gap, status
