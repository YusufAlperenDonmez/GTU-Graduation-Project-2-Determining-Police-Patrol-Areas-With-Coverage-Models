...
# NEW: Custom polygon to reduce the working area [Longitude, Latitude]
USER_POLYGON_COORDS = [
    [-118.310394, 34.198173], 
    [-118.440857, 34.085649], 
    [-118.32962,  33.965003], 
    [-118.108521, 34.008273], 
    [-118.138733, 34.18795], 
    [-118.310394, 34.198173]
]

NUM_BEATS   = 200        # |J|  candidate facility locations
...
def generate_patrol_map():
    ...
    # Load original city boundary
    city_boundary = gpd.read_file(BOUNDARY_FILE_PATH).to_crs(epsg=3857)

    # ── NEW: Apply User Polygon to reduce working area ──
    if USER_POLYGON_COORDS:
        print("   Applying custom user polygon to restrict working area...")
        user_poly = Polygon(USER_POLYGON_COORDS)
        user_boundary = gpd.GeoDataFrame(geometry=[user_poly], crs='EPSG:4326').to_crs(epsg=3857)
        
        # Intersect original LA boundary with the user's polygon
        intersected = gpd.overlay(city_boundary, user_boundary, how='intersection')
        
        # Explicitly force the result back into a GeoDataFrame with correct CRS metadata
        city_boundary = gpd.GeoDataFrame(intersected, geometry='geometry', crs=3857)
    ...
    # Sjoin automatically uses our newly restricted `city_boundary`
    gdf = gpd.sjoin(gdf, city_boundary, how='inner', predicate='within').copy()
    weights = gdf['crime_weight'].values
    n_inc   = len(gdf)
    print(f"   {n_inc:,} incidents loaded within restricted boundary.  ({time.time()-t0:.1f}s)")
    ...
    t2 = time.time()
    # Passes the restricted boundary to only download the needed network chunk
    ...
    beat_coverage_sets  = build_coverage_sets(
        G_metric, beat_nodes, beat_nodes, SERVICE_M
    )
    ...
    # Defensive check: Ensure it is a valid GeoDataFrame and fix invalid topologies
    city_boundary = gpd.GeoDataFrame(city_boundary, geometry='geometry', crs=3857)
    ...
    # FIX: Calculate a massive outer boundary using the city's total bounds
    minx, miny, maxx, maxy = city_boundary.total_bounds
    w, h = maxx - minx, maxy - miny
    pad = max(w, h) * 10  # Push dummy points safely miles away
    ...
    # Append dummy points to the real beat centers for the Voronoi calculation
    vor_inputs = np.vstack([beat_centers, dummy_points])
    vor = Voronoi(vor_inputs)
    
    rows = []
    # Only iterate through the original real beat centers (ignore the last 4 dummies)
    for i in range(NUM_BEATS):
        region_idx = vor.point_region[i]
        region = vor.regions[region_idx]
        
        # Every real beat is now safely enclosed, meaning -1 will not appear here
    ...
    # Explicitly define the geometry column so it doesn't downgrade to a DataFrame
    gdf_voronoi       = gpd.GeoDataFrame(rows, geometry='geometry', crs=3857)
    
    # Clip the extended cells perfectly flush with the city boundary edge
